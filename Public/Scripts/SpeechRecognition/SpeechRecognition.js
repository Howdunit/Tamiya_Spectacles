...
var onUpdateListeningEventHandler = function(eventArgs) {
    ...
    if (script.useKeyword && eventArgs.getKeywordResponses().length>0) {
        ...
        if (eventArgs.isFinalTranscription && script.isLIVE_TRANSCRIPTION) {
            prevKeyword ="";      
        } else {
            ...
            for (var kIterator=0; kIterator<keywords.length; kIterator++) {
                ...
                if (doesKeywordExist(keyword) && script.isLIVE_TRANSCRIPTION||eventArgs.isFinalTranscription && prevKeyword !== keyword) {
